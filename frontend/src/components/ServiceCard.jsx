...
export function ServiceCard({
  ...
  const lastChecked = lastCheckedIso
    ? new Date(lastCheckedIso).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' })
    : '—'

  const badgeClass = statusStyles[status] ?? statusStyles.UNKNOWN

  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        <time className="shrink-0 text-xs text-slate-400 dark:text-slate-500" dateTime={lastCheckedIso ?? undefined}>
          {lastChecked}
        </time>
      </div>
    </article>
